...
<h2 class="main-title"><i class="fa fa-file-text"></i> Add New User</h2>
<br><br>
<div class="big-container-main bag-white">
    {!! Form::open(array('url' => 'users', 'class' => 'big-form')) !!}
        <div class="field" data-title="Username">
            {{Form::text('username', '', ['class' => $errors->has('username') ? ' error' : '', 'placeholder' => '...'])}}
        ...
        <div class="field" data-title="Email">
            {{Form::text('email', '', ['class' => $errors->has('email') ? ' error' : '', 'placeholder' => '...'])}}
            @if ($errors->has('email'))
                <div class="error-msg"><span>Please fill in the required fields</span></div>
            @endif
        </div>
